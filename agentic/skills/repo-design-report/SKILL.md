...
- **报告是成品，不描述研究过程。** 报告里不出现研究过程语言：不写"外围实体"、"外围一句话"、"研究范围: ..."、"先解释三个贯穿全文的词"、"先回答三个问题"这类元叙述和预告式开头；不写验证过程（例如 "本文所有论断均对照源码与模块自带测试验证。测试文件是行为的'地面真相'（tests/...）"）；不引用测试文件、脚本路径、work/ 文件。验证发生在研究阶段，报告只呈现结论。术语在首次出现时自然解释，不预告。
