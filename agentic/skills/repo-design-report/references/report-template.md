...
- **表结构描述只用通用词汇，不自造模糊叠词。** 描述数据结构统一用这套层次分明的词：**领域模型**（设计层面的概念，可能对应一张表，也可能拆成主表+子表，也可能不落地成任何物理表）→ **表**（物理存储单元）→ **字段/列**（表的一列）→ **行/记录**（表的一行数据，是某个领域模型的一个实例）；一对多关系统一说**主表/子表**，子表通过外键字段关联主表（例如 "Workflow Document State 是 Workflow 的子表，通过 parent 字段关联主表，每一行是一个状态的完整配置"）。不要自造"状态行""主档"这类把"表"和"表里的一行"混在一起、或者把某张表英文原名的后缀（如 Master）套用成整类表统称的模糊词。已有的标准词够用："全局注册表""字典表"可以指代那些被多个流程复用的查找表，不需要另造"主档"。如果确实需要一个简称指代"某张表的一行"，首次出现时先用完整句子交代清楚对应关系（"X 概念在数据库层面是 Y 表的一行"），之后才能用简称，跟缩写展开规则一致。
- 偏离本模板必须在报告顶部明确说明。
