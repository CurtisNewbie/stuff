...
- **Functions are breadcrumbs, not content.** Use function names to trace and verify logic, but the report narrates design logic in business terms. Collect the trigger-point → function mapping for the report's evidence appendix; don't put names in the narrative.
...
- **Don't dump identifiers.** The report is design, not a code map. Function names live in the evidence appendix only; the narrative stays logic-first.
