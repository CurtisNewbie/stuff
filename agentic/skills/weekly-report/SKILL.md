...
| **任务描述** | `（进度%）` + PRD/需求链接 + 3W2H bullet points，每个需求一段 |
...
输出 JSON，每条需求包含 `url`（输入的 story URL）、`title`、`description`、`tasks`（名称/估分/状态）、`total_points`。
...
2. **【任务描述列】** 每个需求单独一段：`（进度%）` + `PRD链接：[可点击链接]`（无链接时为 `PRD链接：未提供`）+ What/Why/When/How
