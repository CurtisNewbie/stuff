...
alias gp="git pull"
alias gm="gf && git merge"
alias gl="git log"
alias gds="git diff -p --staged --stat"
alias gd="git diff -p --stat"
# alias gbranch="git for-each-ref --count=10 --sort=-committerdate refs/heads/ --format='%(HEAD) %(align:80)%(color:yellow)%(refname:short)%(color:reset)%(end) - %(color:red)%(objectname:short)%(color:reset) - %(align:40)%(contents:subject)%(end) - %(authorname) (%(color:green)%(committerdate:relative)%(color:reset))'"
