...
alias gbranch="git for-each-ref --count=10 --sort=-committerdate refs/remotes/ --format='%(HEAD) %(align:80)%(color:yellow)%(refname:short)%(color:reset)%(end) - %(align:60)%(contents:subject)%(end) - %(authorname) (%(color:green)%(committerdate:relative)%(color:reset))'"
